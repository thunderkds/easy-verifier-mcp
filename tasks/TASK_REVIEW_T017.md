...
|---|---|---|
| Host integration suite | pass | `PYTHONPATH=src /home/hungnguyenhuu/workspace/training/phuongbui/ai-training/travel_chatbot/.venv/bin/python -m pytest tests/integration -q --tb=short` -> `17 passed, 3 skipped in 8.18s` with one pre-existing Pydantic warning. The skips are the two container proofs and the wrapper-only KPI test. |
| Full regression suite | pass | Compatible interpreter -> `586 passed, 4 skipped in 44.58s`; one upstream Pydantic warning. |
...
combined/score/discovery parity through CLI and MCP, runtime-generated secret
shapes, findings rejection, and report self-containment. Container checks are
