...
| FR-035 | **Hard gate — detect (MCP only).** When `score` is called without picks and a role remains unfilled after FR-031–FR-033 while the repository holds unmatched candidate files, the response must include `needs_input.picks`: per unfilled role at most **20** candidates, each only its repo-relative path and first heading (or first non-empty line), redacted per NFR-010. Ratings computed without picks are returned in the same response; `needs_input` never withholds a result. Absent when nothing qualifies. | US-014, NFR-009 |
