...
**Last updated**: 2026-09-26 (source-discovery amendment — FR-031…FR-040, DDR-0006)
**Status**: Approved
...
| FR-016 | **Amended 2026-09-26 (DDR-0006)**: "sources sought" is now a declared list of **source roles** per dimension (e.g. *lockfile*, *requirements doc*), each satisfiable by any matching file in any supported language; coverage is `roles found / roles declared`, and every role counts in every repository. Original text follows. Each dimension must declare, as static data, the list of context sources it seeks. The engine must compute a **context-coverage score** as the unweighted ratio `found / sought` over that declared list. All sources are equally weighted; the engine must apply no importance judgment. | US-003 |
...
| FR-029a | **Amended 2026-09-26 (DDR-0006)**: sole exception — at a hard gate, a validated gate evaluation contributes to the rating by the capped blend of FR-038, always shown with its parts. Findings-based assessments are still never blended. Original text follows. Where a rating and an assessment both exist for a dimension, the report and both adapters must render them **side by side** with their **divergence**, and must never blend, reconcile, or adjudicate between them. Disagreement between measured rules and an agent's judgment is signal to be surfaced, not an error to resolve. | US-010, US-012 |
...
### Source discovery & agent-assisted evaluation

Any language, no boundary. Rules first; the calling agent is consulted only at **hard gates** —
what the rules cannot detect or cannot rate with confidence (DDR-0006).

| ID | Requirement | Traces to |
|----|-------------|-----------|
| FR-031 | Each dimension must declare its sought sources as **source roles** (static data), each role carrying the glob patterns that satisfy it. Matching is deterministic and ordered. Coverage per FR-016 (amended) is computed over roles, never over individual files. | US-013, US-003 |
| FR-032 | Role patterns must be **language-agnostic by default**: source vs. test by directory and name convention (`test/`, `tests/`, `spec/`, `__tests__/`, `*_test.*`, `*.test.*`, `*.spec.*`, `*Test.*`, `test_*`), lockfiles by name shape (`*.lock`, `*-lock.*`, `*.sum`), CI by location (`.github/workflows/*`, `.gitlab-ci.yml`, …), docs by location and name keyword (`docs/**`, `adr/**`, `*spec*`, `*requirement*`, `*design*`, `*architecture*`). Per-ecosystem pattern sets (v1: Python, JS/TS, Rust, Java) are **additional data**, auto-activated by manifest detection, never a boundary: a repository in an ecosystem with no set must still be evaluated by the generic patterns plus FR-035. A pattern set may add patterns to roles; it may **never** add, remove, or exempt a role. | US-013 |
| FR-033 | A target repository may contain an optional `.easy-verifier.toml` that **adds file paths/globs to existing roles only**. It must not remove roles, mark a role not-applicable, or change a coverage floor or rating rule. Unknown keys are a validation error naming the key. Absence of the file changes nothing (NFR-005). Paths it names are subject to the same read rules as any file (inside the repo, DDR-0002 secret-bearing exclusion, byte budget). | US-013 |
| FR-034 | `score` in both adapters must accept an optional **agent input** document: `{"picks": {role: [repo-relative paths]}, "gate_evaluations": {dimension: {...}}}`. MCP takes it as a tool argument; the CLI as `--agent-input PATH`. Picks are additive only and validated as FR-033 paths are; a pick outside the repo, non-existent, secret-bearing, or naming an unknown role is rejected naming it. The CLI never asks a question and never calls a model — it only replays agent input it is given. | US-014, US-009 |
| FR-035 | **Hard gate — detect (MCP only).** When `score` is called without picks and a role remains unfilled after FR-031–FR-033 while the repository holds unmatched candidate files, the response must include `needs_input.picks`: candidates **grouped by shape** (unfilled roles sharing a candidate pool are listed together, each candidate once per group — amended 2026-09-26 at T027 Stage 4 to cut duplicated tokens ~4×), at most **20** candidates per group and so per role, each only its repo-relative path and first heading (or first non-empty line), redacted per NFR-010. Only files contained in the repository are ever candidates; an escaping symlink is never read. Ratings computed without picks are returned in the same response; `needs_input` never withholds a result. Absent when nothing qualifies. | US-014, NFR-009 |
| FR-036 | **Hard gate — evaluate (MCP only).** After picks are applied (or none were needed), a dimension is at a hard gate when the rules **abstain** (FR-028a) or **any rule input's metric lies within ±10% of its declared threshold** (a threshold of 0 is never borderline — its band has zero width and equality is the rule's best outcome; Supervisor decision at T028 Stage 4, 2026-09-26, restoring the user's "only ask when rules can't decide" intent). For each such dimension the response must include `needs_input.gate_evaluations` naming the dimension, the gate reason, and the evidence-pack references the agent must reason over. At most one picks round and one gate round per evaluation; a `score` call already carrying `gate_evaluations` never emits `needs_input`. | US-014, US-012 |
| FR-037 | A **gate evaluation** must carry `score` (0–100), `confidence` (0–1), and ≥1 `evidence_ref` that resolves in the dimension's pack (FR-015/FR-015a rules apply; a gate evaluation failing them is rejected, naming the field). It is accepted **only** for a dimension currently at a hard gate; an evaluation for any other dimension is rejected. | US-005, US-014 |
...
| FR-022 | **Amended 2026-09-26 (DDR-0006)**: the caller's optional **agent input** (FR-034: picks and gate evaluations) is part of the input — both adapters given the same repository, scope, dimension, *and agent input* must produce byte-equal output. Original text follows. Both adapters must produce **byte-equal** evidence packs and report output for the same repository, scope, and dimension, **after the declared normalization in DDR-0005** (paths made repo-relative, timestamps replaced by a fixed token, report filename excluded). Every field outside that list is compared byte-for-byte with no tolerance. | US-009 |
...
Added during Stage 0.5 requirement grilling for the source-discovery amendment (2026-09-26).
Trigger: live runs against kitchd, bryony, and ai-training abstained on 2–5 of 7 dimensions
because sought sources were fixed Python-/kit-specific filenames.

| # | Question | Status | Owner |
|---|---|--------|-------|
| 16 | May the agent's answer at a borderline/abstaining gate change a rating? | **Superseded same day** (user direction "any language, agent at hard gate to detect and evaluate"): the agent **contributes** via a capped, confidence-weighted blend at hard gates only (FR-036–FR-038); agent-rated where rules abstain. DDR-0003 partially superseded by DDR-0006; FR-029a amended. | thunderkds |
| 17 | MCP-only picks break FR-022 byte-parity. | **Resolved** (user): rules first (CLI path); MCP asks the agent only for what rules cannot settle; picks are replayable via CLI `--sources` so parity holds with picks treated as input (FR-022 amended, FR-034). | thunderkds |
| 18 | Coverage definition once sources are patterns/profiles. | **Resolved** (user): count by **role**, every role in every repo; no per-language exemptions (FR-016 amended, DDR-0006). | thunderkds |
| 19 | When to ask the agent, and at what cost. | **Resolved** (user): only for unfilled roles that have candidates; ≤20 candidates/role, path + first heading, redacted; one round (FR-035). | thunderkds |
| 20 | Language coverage. | **Resolved** (user): **no language boundary** — generic language-agnostic patterns first; Python/JS/TS/Rust/Java pattern sets are extra data; what rules can't classify goes to the agent (FR-032, FR-035). | thunderkds |
| 21 | `.easy-verifier.toml` power. | **Resolved** (user): add paths to roles only; cannot remove roles or change floors (FR-033). | thunderkds |
| 22 | Borderline band for the evaluate gate. | **Resolved 2026-09-26** — user accepted the Supervisor default ±10% of a metric's threshold ("go ahead" after it was presented as the one open item). Tunable as declared data (FR-036). DDR tier kept (not escalated to ADR). | thunderkds |
